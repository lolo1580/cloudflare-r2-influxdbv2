...
During import, select your default InfluxDB datasource. The dashboard also exposes a visible `influxdb_datasource` selector at the top, so you can switch between Grafana InfluxDB datasources after import. Set the `bucket` variable to the exact InfluxDB bucket configured in `.env` as `INFLUX_BUCKET` (for example `cloudflare-r2`). If panels show `could not find bucket`, the selected Grafana `bucket` variable does not match an existing InfluxDB bucket. The dashboard expects Flux queries and the measurements written by this script.
